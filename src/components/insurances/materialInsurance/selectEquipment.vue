...
export default defineComponent({
  ...
  setup(props) {
    const { value: equipment } = useField<Equipment[]>(props.id, props.rules, {
      initialValue: [],
    })
    ...
    const addEquipmentToList = (nonMembersList: Equipment[]) => {
      equipment.value = equipment.value.concat(nonMembersList)
    }
